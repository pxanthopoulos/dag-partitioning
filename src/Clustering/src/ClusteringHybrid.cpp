 ...
 * @brief Implementation of hybrid clustering combining cycle detection and forbidden edges
 */
...
bool ClusteringHybrid::checkLargeDegrees(uint64_t from, uint64_t to) const {
    ...
    if (workingGraph.adj[from].size() > maxDegree) return true;  // Large out-degree
    if (workingGraph.inDegree[to] > maxDegree) return true;      // Large in-degree
    return false;
}

void ClusteringHybrid::bookKeepingForForbiddenEdges(const std::vector<uint64_t> &topLevels,
                                                    uint64_t node,
    ...
    for (const auto &[neighborIdInternal, edgeWeightInternal, isSuccessorInternal]: sortedNeighbors) {
        ...
        uint64_t diffInternal = (topLevels[node] > topLevels[neighborIdInternal]) ?
