 ...
 * @brief Implementation of acyclic clustering with cycle detection as described in Section 4.1.2
 *
 * This class implements a less restrictive clustering algorithm than the forbidden edges approach.
 * It maintains the top-level difference constraint but uses explicit cycle detection instead of
 * forbidden edge rules to ensure acyclicity.
 */
...
private:
     ...
     *
     * This is a verification function that constructs the coarsened graph and checks
     * for cycles using full graph traversal. Used as a safety check to ensure the
     * faster cycle detection didn't miss anything.
     *
     ...
     *
     * Implements Algorithm 2 from the paper, processing nodes in topological order
     * and using detectCycle() to check if merging nodes would create cycles.
     *
...
protected:
    /**
     * @brief Checks if merging nodes `from` and `to` (directed graph) would create a cycle
     *
...
public:
     ...
     */
    ClusteringCycleDetection(const Graph &graph, uint64_t maxRounds, uint64_t minVertices, double vertexRatio);
};
