...
void BoundaryFM::insertMovableNeighborsIntoHeaps(
    ...
    if (checkOutNeighbors) { // Node moved V1->V0, check out-neighbors
        for (const auto &[nodeId, _] : workingGraph.adj[movedNodeId]) {
            ...
            // it
            if (inHeap[nodeId] == 1 || currentBisectionInfo[nodeId] == 0)
                continue;
        ...
        for (const auto &[nodeId, _] : workingGraph.revAdj[movedNodeId]) {
            // If the node has already been in the heap previously, don't re-add
            // it
            if (inHeap[nodeId] == 1 || currentBisectionInfo[nodeId] == 1)
                continue;
...
bool BoundaryFM::confirmMove(uint8_t moveFromV0, uint64_t movedNodeId,
...
bool BoundaryFM::onePassRefinement() {
    ...
    while ((!heapV0.empty() || !heapV1.empty()) &&
        ...
        // If node already moved (or marked move to signify unmovable node),
        // skip
        if (moved[nodeId] == 1)
            continue;
        ...
        if (moveFromV0 == 1) {
            ...
            // If an out-neighbor of the moved nodeId is in V1 and is not yet
            // moved, (if it has already been moved, we don't care if it is
            // movable or not) Then the neighbor becomes unmovable since it has
            // one in-neighbor (the moved nodeId) that is in V1
            for (const auto &[neighborId, _] : workingGraph.adj[nodeId]) {
            ...
            // If an in-neighbor of the moved nodeId is in V0 and is not already
            // movable, (if it has already been marked movable by the initial
            // check, we don't recheck) Then the neighbor might become movable
            // if its only out-neighbor in V0 was the moved node
            insertMovableNeighborsIntoHeaps(initialBisectionInfoTemp, heapV0,
                                            heapV1, inHeap, nodeId, false);
        } else {
            // Moving from V1 to V0 can make V0 nodes unmovable
            // If an in-neighbor of the moved nodeId is in V0 and is not yet
            // moved, (if it has already been moved, we don't care if it is
            // movable or not) Then the neighbor becomes unmovable since it has
            // one out-neighbor (the moved nodeId) that is in V0
            for (const auto &[neighborId, _] : workingGraph.revAdj[nodeId]) {
                if (moved[neighborId] == 0 &&
            ...
            // If an out-neighbor of the moved nodeId is in V1 and is not
            // already movable, (if it has already been marked movable by the
            // initial check, we don't recheck) Then the neighbor might become
            // movable if its only in-neighbor in V1 was the moved node
            insertMovableNeighborsIntoHeaps(initialBisectionInfoTemp, heapV0,
                                            heapV1, inHeap, nodeId, true);
        }
    }
