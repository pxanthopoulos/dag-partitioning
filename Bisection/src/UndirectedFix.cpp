...
void UndirectedFix::graphToCSRFormat(int64_t edgeNumber, std::vector<int64_t> &nodeNeighborsOffset,
                                     std::vector<int64_t> &nodeNeighbors, std::vector<int64_t> &edgeWeights,
...
std::vector<bool> UndirectedFix::getUndirectedBisectionScotch() const {
    ...
    int ret = SCOTCH_graphBuild(&scotchGraph, 0, (int64_t) workingGraph.size, nodeNeighborsOffset.data(),
                                nodeNeighborsOffset.data() + 1, nodeWeights.data(), nullptr, edgeNumber * 2,
