...
metric_dict_diff = {
    "memory_footprint": "Memory Footprint Difference %",
}
metric_dict_absolute = {
    "memory_footprint": "Memory Footprint in KB",
}
titles_diff = {
    "memory_footprint": "Memory Footprint Difference Percentage (Current Implementation - Original)",
}
titles_absolute = {
    "memory_footprint": "Memory Footprint (KB)",
}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="100,250,500",
        help="Comma-separated ratios (integers >= 100) (default: 100,250,500)",
    )
    parser.add_argument(
        "--sizes",
        type=validate_sizes,
        default="10,100,1000,5000,10000,50000",
...
def parse_trace_file(filepath, ratios, sizes, max_base, runs):
    ...
    for line in trace_data.strip().split("\n"):
        if line.startswith("Error"):
            ...
            clustering, bisection, refinement, metric1, metric2, metric3, metric4 = (
                line.split(",")
            )
                ...
                for p in partitions:
                    ...
                    for run in range(1, args.runs + 1):
                        ...
                        for algorithm in ALGORITHMS:
                            ...
                            try:
                                with (
                                    open(args.baseline_trace_file, "a+b") as f,
                                    open(args.subprocess_log, "a") as log_f,
                                    open(massif_txt_path, "w+") as massif_f,
                                ):
                                    subprocess.run(
