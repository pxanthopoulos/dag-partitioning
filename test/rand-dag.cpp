...
class DAGGenerator {
private:
  std::vector<std::vector<std::pair<uint64_t, int64_t>>> adj;
  int size;
  ...
  bool dfs(int v, std::vector<int> &visited, std::vector<int> &recursionStack) {
    if (visited[v] == 0) {
      visited[v] = 1;
      recursionStack[v] = 1;
...
public:
  DAGGenerator(int n) : size(n), gen(rd()) { adj.resize(n); }
  ...
  int generate() {
    ...
    int targetEdges = (int)((double)size * 1.5);
    int attempts = 0;
    ...
    for (const auto &edge : edges) {
      adj[edge.first].push_back({edge.second, 1});
    }
